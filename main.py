...
class DerivBot:
    ...
    async def start(self):
        logger.info("=" * 65)
        logger.info("  DERIV OVER/UNDER BOT v11 — OVER 4 / UNDER 5 ONLY")
        ...
        logger.info("")
        logger.info("  v11 Key Changes from v10:")
        logger.info("    - ONLY Over 4 and Under 5 — no more Over 5/6/7/8!")
        logger.info("    - Both have 50% natural probability — FAIR contracts")
        logger.info("    - ~95% payout — breakeven at 52.6% win rate")
        logger.info("    - z-score 2.0 minimum (was 3.0)")
        logger.info("    - 3% minimum EV (was 8%)")
        logger.info("    - 52% minimum confidence (must be above 50% natural)")
        logger.info("    - Less Bayesian shrinkage (prior=50, was 100)")
        logger.info("    - No random market exploration — stay on same market")
        logger.info("    - Signal freshness 15s (was 5s)")
        logger.info("")
