...
def evaluate(symbol: str,
    ...
    if sweep_bias == 'BULLISH':
        ...
        try:
            ...
            fib_report = build_fib_report(df_h1=df_h1, df_h4=df_h4, current_price=current_price)
            fib_check = check_fib_confluence(current_price, "BUY", fib_report, pip_size)
    ...
    if sweep_bias == 'BEARISH':
        ...
        try:
            from backtest.fib_builder import build_fib_report, check_fib_confluence
            fib_report = build_fib_report(df_h1=df_h1, df_h4=df_h4, current_price=current_price)
            fib_check = check_fib_confluence(current_price, "SELL", fib_report, pip_size)
            if fib_check['fib_bonus'] > 0:
                score += fib_check['fib_bonus']
                confluence.extend(fib_check['confluence'])
        except Exception:
