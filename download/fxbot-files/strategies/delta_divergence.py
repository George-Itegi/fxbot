...
MIN_SCORE     = 75  # v4.7: Raised from 70 (was too easy)
VERSION       = "1.0"
...
def _detect_bearish_divergence(df: pd.DataFrame, swings: dict,
    ...
    if len(delta_at_swings) < 2:
        # v4.7 FIX: Remove EXTREME fallback — it fired on sparse data
        # with zero historical comparison. Just any negative delta
        # while price is high is NOT real divergence.
        return None
...
def _detect_bullish_divergence(df: pd.DataFrame, swings: dict,
    ...
    if len(delta_at_swings) < 2:
        # v4.7 FIX: Remove EXTREME fallback — same reason as bearish
        return None
...
def evaluate(symbol: str,
    ...
    if bear_div and (bull_div is None or bear_div.get('delta_strength') == 'EXTREME'):
        ...
        # v4.7 FIX: No fallback — the old near-swing+opposing-delta fallback
        # was a false signal factory. Only real divergence passes.
        pass
