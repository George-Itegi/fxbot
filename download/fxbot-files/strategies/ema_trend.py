...
def evaluate(symbol: str,
    ...
    # ── INSTITUTIONAL GATE (v4.7) ─────────────────────────
    # EMA_TREND fires on lagging EMA crossovers with NO institutional
    # confirmation. Require at least ONE institutional signal before
    # any trade direction is evaluated.
    # Returns: (passed: bool, reason: str)
    def _check_inst_gate(d: str) -> tuple:
        if not smc_report:
            return False, "no_smc_data"
        nearest_ob = smc_report.get('nearest_ob', {})
        if nearest_ob:
            ob_type = nearest_ob.get('type', '')
            ob_dist = float(nearest_ob.get('pips_to_ob', 999))
            if (d == 'BUY' and ob_type == 'BULLISH' and ob_dist < 25) or \
               (d == 'SELL' and ob_type == 'BEARISH' and ob_dist < 25):
                return True, f"OB_{ob_type}_{ob_dist:.0f}p"
        last_sweep = smc_report.get('last_sweep')
        if last_sweep:
            sweep_bias = last_sweep.get('bias', '')
            if ((d == 'BUY' and sweep_bias == 'BULLISH') or
                (d == 'SELL' and sweep_bias == 'BEARISH')):
                return True, "SWEEP_REVERSED"
        bos = smc_report.get('structure', {}).get('bos', {})
        if bos:
            bos_type = bos.get('type', '')
            if ((d == 'BUY' and 'BULL' in bos_type.upper()) or
                (d == 'SELL' and 'BEAR' in bos_type.upper())):
                return True, f"BOS_{bos_type}"
        return False, "no_inst_signal"
    ...
    if h4_bull and h1_bull:
        # v4.7: Institutional gate — must have OB/sweep/BOS before scoring
        inst_ok, inst_reason = _check_inst_gate('BUY')
        if not inst_ok:
            return None  # No institutional confirmation = no trade
        log.info(f"[{STRATEGY_NAME}] {symbol} BUY — inst gate PASSED: {inst_reason}")
        score      = 0
        confluence = []
    ...
    if h4_bear and h1_bear:
        # v4.7: Institutional gate — must have OB/sweep/BOS before scoring
        inst_ok, inst_reason = _check_inst_gate('SELL')
        if not inst_ok:
            return None  # No institutional confirmation = no trade
        log.info(f"[{STRATEGY_NAME}] {symbol} SELL — inst gate PASSED: {inst_reason}")
        score      = 0
        confluence = []
