...
def evaluate(symbol: str,
    ...
    # ── Step 4.5: INSTITUTIONAL GATE (v4.7) ────────────────
    # Trend Continuation is pure EMA riding without institutional
    # confirmation. Require at least ONE institutional signal.
    ...
    if smc_report:
        ...
        if nearest_ob:
            ...
            if (direction == 'BUY' and ob_type == 'BULLISH' and ob_dist < 30) or \
               (direction == 'SELL' and ob_type == 'BEARISH' and ob_dist < 30):
                institutional_ok = True
                confluence.append(f"INST_OB_{ob_type}")
        if not institutional_ok:
            last_sweep = smc_report.get('last_sweep')
            if last_sweep:
                sweep_bias = last_sweep.get('bias', '')
                if ((direction == 'BUY' and sweep_bias == 'BULLISH') or
                    (direction == 'SELL' and sweep_bias == 'BEARISH')):
                    institutional_ok = True
                    confluence.append("INST_SWEEP")
        if not institutional_ok:
            bos = smc_report.get('structure', {}).get('bos', {})
            if bos:
                bos_type = bos.get('type', '')
                if ((direction == 'BUY' and 'BULL' in bos_type.upper()) or
                    (direction == 'SELL' and 'BEAR' in bos_type.upper())):
                    institutional_ok = True
                    confluence.append(f"INST_BOS_{bos_type}")
    if not institutional_ok:
        log.debug(f"[{STRATEGY_NAME}] {symbol} — inst gate FAILED (no OB/sweep/BOS)")
        return None
