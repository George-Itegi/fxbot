...
from config import DEFAULT_SYMBOL, OVER_BARRIER, UNDER_BARRIER, CONTRACT_DURATION
...
async def collect_training_data(symbol: str, num_ticks: int = 5000) -> tuple:
    ...
    for i in range(len(all_ticks)):
        tick = all_ticks[i]
        agg.add_tick(tick["epoch"], tick["quote"])
        ...
        # Determine label: did ANY of the next N ticks have digit > barrier?
        over_hit = False
        under_hit = False
        
        for j in range(i + 1, i + look_ahead + 1):
            future_tick = all_ticks[j]
            price_str = f"{future_tick['quote']:.2f}"
            digit = int(price_str[-1])
            if digit > OVER_BARRIER:
                over_hit = True
            if digit < UNDER_BARRIER:
                under_hit = True
        
        # We'll train for Over prediction (label=1 means Over hits)
        label = 1 if over_hit else 0
